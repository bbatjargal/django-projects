...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>Foreign Server Dialog &#8212; pgAdmin 4 3.1 documentation</title>
  ...
  <div class="section" id="foreign-server-dialog">
<span id="id1"></span><h1><span class="target" id="index-0"></span>Foreign Server Dialog<a class="headerlink" href="#foreign-server-dialog" title="Permalink to this headline">¶</a></h1>
<p>Use the <em>Foreign Server</em> dialog to create a foreign server. A foreign server typically encapsulates connection information that a foreign-data wrapper uses to access an external data resource. Each foreign data wrapper may connect to a different foreign server; in the <em>pgAdmin</em> tree control, expand the node of the applicable foreign data wrapper to launch the <em>Foreign Server</em> dialog.</p>
<p>The <em>Foreign Server</em> dialog organizes the development of a foreign server through the following dialog tabs: <em>General</em>, <em>Definition</em>, <em>Options</em>, and <em>Security</em>. The <em>SQL</em> tab displays the SQL code generated by dialog selections.</p>
<img alt="Foreign server dialog general tab" src="_images/foreign_server_general.png" />
<p>Use the fields in the <em>General</em> tab to identify the foreign server:</p>
<ul class="simple">
<li>Use the <em>Name</em> field to add a descriptive name for the foreign server. The name will be displayed in the <em>pgAdmin</em> tree control. It must be unique within the database.</li>
<li>Use the drop-down listbox next to <em>Owner</em> to select a role.</li>
<li>Store notes about the foreign server in the <em>Comment</em> field.</li>
</ul>
<p>Click the <em>Definition</em> tab to continue.</p>
<img alt="Foreign server dialog definition tab" src="_images/foreign_server_definition.png" />
<p>Use the fields in the <em>Definition</em> tab to set parameters:</p>
<ul class="simple">
<li>Use the <em>Type</em> field to specify a server type.</li>
<li>Use the <em>Version</em> field to specify a server version.</li>
</ul>
<p>Click the <em>Options</em> tab to continue.</p>
<img alt="Foreign server dialog options tab" src="_images/foreign_server_options.png" />
<p>Use the fields in the <em>Options</em> tab to specify options. Click the <em>Add</em> button to create an option clause for the foreign server.</p>
<ul class="simple">
<li>Specify the option name in the <em>Option</em> field.</li>
<li>Provide a corresponding value in the <em>Value</em> field.</li>
</ul>
<p>Click <em>Add</em> to create each additional clause; to discard an option, click the trash icon to the left of the row and confirm deletion in the <em>Delete Row</em> popup.</p>
<p>Click the <em>Security</em> tab to continue.</p>
<img alt="Foreign server dialog security tab" src="_images/foreign_server_security.png" />
<p>Use the <em>Security</em> tab to assign security privileges to the foreign server. Click <em>Add</em> before you assign a set of privileges.</p>
<ul class="simple">
...
<li>Select the name of the role from the drop-down listbox in the <em>Grantor</em> field. The default grantor is the owner of the foreign server. This is a required field.</li>
</ul>
<p>Click <em>Add</em> to assign a new set of privileges; to discard a privilege, click the trash icon to the left of the row and confirm deletion in the <em>Delete Row</em> dialog.</p>
<p>Click the <em>SQL</em> tab to continue.</p>
<p>Your entries in the <em>Foreign Server</em> dialog generate a SQL command (see an example below). Use the <em>SQL</em> tab for review; revisit or switch tabs to make any changes to the SQL command.</p>
<p><strong>Example</strong></p>
<p>The following is an example of the sql command generated by user selections in the <em>Foreign Server</em> dialog:</p>
<img alt="Foreign server dialog sql tab" src="_images/foreign_server_sql.png" />
<p>The example shown demonstrates creating a foreign server for the foreign data wrapper <em>hdfs_fdw</em>. It has the name <em>hdfs_server</em>; its type is <em>hiveserver2</em>. Options for the foreign server include a host and a port.</p>
<ul class="simple">
