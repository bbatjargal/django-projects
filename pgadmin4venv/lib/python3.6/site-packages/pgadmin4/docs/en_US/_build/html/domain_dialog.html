...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>Domain Dialog &#8212; pgAdmin 4 3.1 documentation</title>
  ...
  <div class="section" id="domain-dialog">
<span id="id1"></span><h1><span class="target" id="index-0"></span>Domain Dialog<a class="headerlink" href="#domain-dialog" title="Permalink to this headline">¶</a></h1>
<p>Use the <em>Domain</em> dialog to define a domain. A domain is a data type definition that may constrain permissible values. Domains are useful when you are creating multiple tables that contain comparable columns; you can create a domain that defines constraints that are common to the columns and re-use the domain definition when creating the columns, rather than individually defining each set of constraints.</p>
<p>The <em>Domain</em> dialog organizes the development of a domain through the following tabs: <em>General</em>, <em>Definition</em>, <em>Constraints</em>, and <em>Security</em>. The <em>SQL</em> tab displays the SQL code generated by dialog selections.</p>
<img alt="Domain dialog general tab" src="_images/domain_general.png" />
<p>Use the fields on the <em>General</em> tab to identify a domain:</p>
<ul class="simple">
<li>Use the <em>Name</em> field to add a descriptive name for the domain. The name will be displayed in the <em>pgAdmin</em> tree control.</li>
<li>Use the drop-down listbox next to <em>Owner</em> to select a role that will own the domain.</li>
<li>Select the name of the schema in which the domain will reside from the drop-down listbox in the <em>Schema</em> field.</li>
<li>Store notes about the domain in the <em>Comment</em> field.</li>
</ul>
<p>Click the <em>Definition</em> tab to continue.</p>
<img alt="Domain dialog definition tab" src="_images/domain_definition.png" />
<p>Use the fields in the <em>Definition</em> tab to describe the domain:</p>
<ul class="simple">
<li>Use the drop-down listbox next to <em>Base type</em> to specify a data type.</li>
<li>Use the context-sensitive <em>Length</em> field to specify a numeric length for a numeric type.</li>
<li>Use the context-sensitive <em>Precision</em> field to specify the total count of significant digits for a numeric type.</li>
<li>Specify a default value for the domain data type in the <em>Default</em> field. The data type of the default expression must match the data type of the domain. If no default value is specified, then the default value is the null value.</li>
<li>Move the <em>Not Null</em> switch to specify the values of this domain are prevented from being null.</li>
<li>Use the drop-down listbox next to <em>Collation</em> to apply a collation cast. If no collation is specified, the underlying data type’s default collation is used. The underlying type must be collatable if COLLATE is specified.</li>
</ul>
<p>Click the <em>Constraints</em> tab to continue.</p>
<img alt="Domain dialog constraints tab" src="_images/domain_constraints.png" />
<p>Use the fields in the <em>Constraints</em> tab to specify rules for the domain. Click the <em>Add</em> icon (+) to set constraints:</p>
<ul class="simple">
<li>Use the <em>Name</em> field to specify a name for the constraint.</li>
<li>Use the <em>Check</em> field to provide an expression for the constraint.</li>
<li>Use the <em>Validate</em> checkbox to determine whether the constraint will be validated. The default checkbox is checked and sets a validation requirement.</li>
</ul>
<p>A CHECK clause specifies an integrity test which values of the domain must satisfy. Each constraint must be an expression that produces a Boolean result. Use the key word VALUE to refer to the value being tested. Expressions evaluating to TRUE or UNKNOWN succeed. If the expression produces a FALSE result, an error is reported and the value is not allowed to be converted to the domain type.  A CHECK expression cannot contain subqueries nor refer to variables other than VALUE.  If a domain has multiple CHECK constraints, they will be tested in alphabetical order by name.</p>
<p>Click the <em>Add</em> icon (+) to set additional constraints; to discard a constraint, click the trash icon to the left of the row and confirm deletion in the <em>Delete Row</em> popup.</p>
<p>Click the <em>Security</em> tab to continue.</p>
<img alt="Domain dialog security tab" src="_images/domain_security.png" />
<p>Use the <em>Security Labels</em> panel to assign security labels. Click the <em>Add</em> icon (+) to add a label:</p>
<ul class="simple">
...
<p>Click the <em>Add</em> icon (+) to specify each additional label; to discard a label, click the trash icon to the left of the row and confirm deletion in the <em>Delete Row</em> popup.</p>
<p>Click the <em>SQL</em> tab to continue.</p>
<p>Your entries in the <em>Domain</em> dialog generate a SQL command (see an example below). Use the <em>SQL</em> tab for review; revisit or switch tabs to make any changes to the SQL command.</p>
<p><strong>Example</strong></p>
<p>The following is an example of the sql command generated by selections made in the <em>Domain</em> dialog:</p>
<img alt="Domain dialog sql tab" src="_images/domain_sql.png" />
<p>The example shown demonstrates creating a domain named <em>minimum-wage</em> that confirms that the value entered is greater than or equal to <em>7.25</em>.</p>
<ul class="simple">
