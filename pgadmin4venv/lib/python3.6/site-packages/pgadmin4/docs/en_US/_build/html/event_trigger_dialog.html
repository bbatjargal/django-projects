...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>Event Trigger Dialog &#8212; pgAdmin 4 3.1 documentation</title>
  ...
  <div class="section" id="event-trigger-dialog">
<span id="id1"></span><h1><span class="target" id="index-0"></span>Event Trigger Dialog<a class="headerlink" href="#event-trigger-dialog" title="Permalink to this headline">¶</a></h1>
<p>Use the <em>Domain Trigger</em> dialog to define an event trigger. Unlike regular triggers, which are attached to a single table and capture only DML events, event triggers are global to a particular database and are capable of capturing DDL events.  Like regular triggers, event triggers can be written in any procedural language that includes event trigger support, or in C, but not in SQL.</p>
<p>The <em>Domain Trigger</em> dialog organizes the development of a event trigger through the following dialog tabs: <em>General</em>, <em>Definition</em>, and <em>Security Labels</em>. The <em>SQL</em> tab displays the SQL code generated by dialog selections.</p>
<img alt="Event trigger dialog general tab" src="_images/event_trigger_general.png" />
<p>Use the fields in the <em>General</em> tab to identify the event trigger:</p>
<ul class="simple">
<li>Use the <em>Name</em> field to add a descriptive name for the event trigger. The name will be displayed in the <em>pgAdmin</em> tree control.</li>
<li>Use the drop-down listbox next to <em>Owner</em> to specify the owner of the event trigger.</li>
<li>Store notes about the event trigger in the <em>Comment</em> field.</li>
</ul>
<p>Click the <em>Definition</em> tab to continue.</p>
<img alt="Event trigger dialog definition tab" src="_images/event_trigger_definition.png" />
<p>Use the fields in the <em>Definition</em> tab to define the event trigger:</p>
<ul class="simple">
<li>Select a radio button in the <em>Enabled Status</em> field to specify a status for the trigger: <em>Enable</em> <em>Disable</em>, <em>Replica</em> <em>Always</em>.</li>
<li>Use the drop-down listbox next to <em>Trigger function</em> to specify an existing function.  A trigger function takes an empty argument list, and returns a value of type event_trigger.</li>
<li>Select a radio button in the <em>Events</em> field to specify when the event trigger will fire: <em>DDL COMMAND START</em>, <em>DDL COMMAND END</em>, or <em>SQL DROP</em>.</li>
<li>Use the <em>When</em> field to write a condition for the event trigger that must be satisfied before the event trigger can execute.</li>
</ul>
<p>Click the <em>Security Labels</em> tab to continue.</p>
<img alt="Event trigger dialog security tab" src="_images/event_trigger_security.png" />
<p>Use the <em>Security</em> tab to define security labels applied to the trigger. Click the <em>Add</em> icon (+) to add each security label.</p>
<ul class="simple">
...
<p>Your entries in the <em>Domain Trigger</em> dialog generate a generate a SQL command. Use the <em>SQL</em> tab for review; revisit or switch tabs to make any changes to the SQL command.</p>
<p><strong>Example</strong></p>
<p>The following is an example of the sql command generated by user selections in the <em>Domain Trigger</em> dialog:</p>
<img alt="Event trigger dialog sql tab" src="_images/event_trigger_sql.png" />
<p>The command creates an event trigger named <em>accounts</em> that invokes the procedure named <em>acct_due</em>.</p>
<ul class="simple">
