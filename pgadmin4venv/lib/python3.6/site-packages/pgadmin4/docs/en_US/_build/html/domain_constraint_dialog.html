...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>Domain Constraints Dialog &#8212; pgAdmin 4 3.1 documentation</title>
  ...
  <div class="section" id="domain-constraints-dialog">
<span id="domain-constraint-dialog"></span><h1><span class="target" id="index-0"></span>Domain Constraints Dialog<a class="headerlink" href="#domain-constraints-dialog" title="Permalink to this headline">¶</a></h1>
<p>Use the <em>Domain Constraints</em> dialog to create or modify a domain constraint. A domain constraint confirms that the values provided for a domain meet a defined criteria. The <em>Domain Constraints</em> dialog implements options of the ALTER DOMAIN command.</p>
<p>The <em>Domain Constraints</em> dialog organizes the development of a domain constraint through the following dialog tabs: <em>General</em> and <em>Definition</em>. The <em>SQL</em> tab displays the SQL code generated by dialog selections.</p>
<img alt="Domain constraint dialog general tab" src="_images/domain_constraint_general.png" />
<p>Use the fields in the <em>General</em> tab to identify the domain constraint:</p>
<ul class="simple">
<li>Use the <em>Name</em> field to add a descriptive name for the constraint. The name will be displayed in the <em>pgAdmin</em> tree control.</li>
<li>Store notes about the constraint in the <em>Comment</em> field.</li>
</ul>
<p>Click the <em>Definition</em> tab to continue.</p>
<img alt="Domain constraint dialog general tab" src="_images/domain_constraint_definition.png" />
<p>Use the fields in the <em>Definition</em> tab to define the domain constraint:</p>
<ul class="simple">
<li>Use the <em>Check</em> field to provide a CHECK expression. A CHECK expression specifies a constraint that the domain must satisfy. A constraint must produce a Boolean result; include the key word VALUE to refer to the value being tested. Only those expressions that evaluate to TRUE or UNKNOWN will succeed. A CHECK expression cannot contain subqueries or refer to variables other than VALUE. If a domain has multiple CHECK constraints, they will be tested in alphabetical order.</li>
<li>Move the <em>Validate?</em> switch to the <em>No</em> position to mark the constraint NOT VALID. If the constraint is marked NOT VALID, the constraint will not be applied to existing column data. The default value is <em>Yes</em>.</li>
</ul>
<p>Click the <em>SQL</em> tab to continue.</p>
<p>Your entries in the <em>Domain Constraints</em> dialog generate a SQL command (see an example below). Use the <em>SQL</em> tab for review; revisit or switch tabs to make any changes to the SQL command.</p>
<p><strong>Example</strong></p>
<p>The following is an example of the sql command generated by user selections in the <em>Domain Constraints</em> dialog:</p>
<img alt="Domain constraint dialog general tab" src="_images/domain_constraint_sql.png" />
<p>The example shown demonstrates creating a domain constraint on the domain <em>timesheets</em> named <em>weekday</em>. It constrains a value to equal <em>Friday</em>.</p>
<ul class="simple">
