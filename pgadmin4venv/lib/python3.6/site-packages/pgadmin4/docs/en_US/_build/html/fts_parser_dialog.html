...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>FTS Parser Dialog &#8212; pgAdmin 4 3.1 documentation</title>
    ...
    <link rel="next" title="FTS Template Dialog" href="fts_template_dialog.html" />
    <link rel="prev" title="FTS Dictionary Dialog" href="fts_dictionary_dialog.html" /> 
  </head><body>
  ...
  <div class="section" id="fts-parser-dialog">
<span id="id1"></span><h1><span class="target" id="index-0"></span>FTS Parser Dialog<a class="headerlink" href="#fts-parser-dialog" title="Permalink to this headline">¶</a></h1>
<p>Use the <em>FTS Parser</em> dialog to create a new text search parser. A text search parser defines a method for splitting a text string into tokens and assigning types (categories) to the tokens.</p>
<p>The <em>FTS Parser</em> dialog organizes the development of a text search parser through the following dialog tabs: <em>General</em>, and <em>Definition</em>. The <em>SQL</em> tab displays the SQL code generated by dialog selections.</p>
<img alt="FTS parser dialog general tab" src="_images/fts_parser_general.png" />
<p>Use the fields in the <em>General</em> tab to identify a text search parser:</p>
<ul class="simple">
<li>Use the <em>Name</em> field to add a descriptive name for the parser. The name will be displayed in the <em>pgAdmin</em> tree control.</li>
<li>Select the name of the schema in which the parser will reside from the drop-down listbox in the <em>Schema</em> field.</li>
<li>Store notes about the domain in the <em>Comment</em> field.</li>
</ul>
<p>Click the <em>Definition</em> tab to continue.</p>
<img alt="FTS parser dialog definition tab" src="_images/fts_parser_definition.png" />
<p>Use the fields in the <em>Definition</em> tab to define parameters:</p>
<ul class="simple">
<li>Use the drop-down listbox next to <em>Start function</em> to select the name of the function that will initialize the parser.</li>
<li>Use the drop-down listbox next to <em>Get next token function</em> to select the name of the function that will return the next token.</li>
<li>Use the drop-down listbox next to <em>End function</em> to select the name of the function that is called when the parser is finished.</li>
<li>Use the drop-down listbox next to <em>Lextypes function</em> to select the name of the lextypes function for the parser. The lextypes function returns an array that contains the id, alias, and a description of the tokens used by the parser.</li>
<li>Use the drop-down listbox next to <em>Headline function</em> to select the name of the headline function for the parser. The headline function returns an excerpt from the document in which the terms of the query are highlighted.</li>
</ul>
<p>Click the <em>SQL</em> tab to continue.</p>
<img alt="FTS parser dialog sql tab" src="_images/fts_parser_sql.png" />
<p>Your entries in the <em>FTS Parser</em> dialog generate a generate a SQL command. Use the <em>SQL</em> tab for review; revisit or switch tabs to make any changes to the SQL command.</p>
<ul class="simple">
...
<li class="toctree-l2 current"><a class="current reference internal" href="#">FTS Parser Dialog</a></li>
