...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>Installing pgAgent &#8212; pgAdmin 4 3.1 documentation</title>
...
<p>pgAgent runs as a daemon on Unix systems, and a service on Windows systems.  In most cases it will run on the database server itself - for this reason, pgAgent is not automatically configured when pgAdmin is installed. In some cases however, it may be preferable to run pgAgent on multiple systems, against the same database; individual jobs may be targeted at a particular host, or left for execution by any host. Locking prevents execution of the same instance of a job by multiple hosts.</p>
...
<p>Before using pgAdmin to manage pgAgent, you must create the pgAgent extension in the maintenance database registered with pgAdmin.  To install pgAgent on a PostgreSQL host, connect to the <em>postgres</em> database, and navigate  through the <em>Tools</em> menu to open the Query tool.  For server versions 9.1 or later, and pgAgent 3.4.0 or later, enter the following command in the query window, and click the <em>Execute</em> icon:</p>
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="n">CREATE</span> <span class="n">EXTENSION</span> <span class="n">pgagent</span><span class="p">;</span>
</pre></div>
</div>
<p>This command will create a number of tables and other objects in a schema called ‘pgagent’.</p>
<p>The database must also have the pl/pgsql procedural language installed - use the PostgreSQL CREATE LANGUAGE command to install pl/pgsql if necessary.  To install pl/pgsql, enter the following command in the query window, and click the <em>Execute</em> icon:</p>
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="n">CREATE</span> <span class="n">LANGUAGE</span> <span class="n">plpgsql</span><span class="p">;</span>
</pre></div>
</div>
<dl class="docutils">
<dt>If you are using an earlier version of PostgreSQL or pgAgent, use the <em>Open file</em> icon on the Query Tool toolbar to open a browser window and locate the <em>pgagent.sql</em> script. The installation script is installed by pgAdmin, and the installation location varies from operating system to operating system:</dt>
<dd><ul class="first last simple">
<li>On Windows, it is usually located under <em>C:Program filespgAdmin III</em> (or <em>C:Program filesPostgreSQL8.xpgAdmin III</em> if installed with the PostgreSQL server installer).</li>
<li>On Linux, it is usually located under <em>/usr/local/pgadmin3/share/pgadmin3</em> or <em>/usr/share/pgadmin3</em>.</li>
</ul>
</dd>
</dl>
<p>After loading the file into the Query Tool, click the <em>Execute</em> icon to execute the script.  The script will create a number of tables and other objects in a schema named <em>pgagent</em>.</p>
</div>
...
<p>To install the pgAgent daemon on a Unix system, you will normally need to have root privileges to modify the system startup scripts.  Modifying system startup scripts is quite system-specific so you should consult your system documentation for further information.</p>
<p>The program itself takes few command line options, most of which are only needed for debugging or specialised configurations:</p>
...
<p>The connection string is a standard PostgreSQL libpq connection string (see the <a class="reference external" href="http://www.postgresql.org/docs/current/static/libpq.html#libpq-connect">PostgreSQL documentation on the connection string</a> for further details). For example, the following command line will run pgAgent against a server listening on the localhost, using a database called ‘pgadmin’, connecting as the user ‘postgres’:</p>
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="o">/</span><span class="n">path</span><span class="o">/</span><span class="n">to</span><span class="o">/</span><span class="n">pgagent</span> <span class="n">hostaddr</span><span class="o">=</span><span class="mf">127.0</span><span class="o">.</span><span class="mf">0.1</span> <span class="n">dbname</span><span class="o">=</span><span class="n">postgres</span> <span class="n">user</span><span class="o">=</span><span class="n">postgres</span>
</pre></div>
</div>
</div>
...
<p>pgAgent can install itself as a service on Windows systems.  The command line options available are similar to those on Unix systems, but include an additional parameter to tell the service what to do:</p>
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="n">Usage</span><span class="p">:</span>
...
<p>The service may be quite simply installed from the command line as follows (adjust the path as required):</p>
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="s2">&quot;C:\Program Files\pgAdmin III\pgAgent&quot;</span> <span class="n">INSTALL</span> <span class="n">pgAgent</span> <span class="o">-</span><span class="n">u</span> <span class="n">postgres</span> <span class="o">-</span><span class="n">p</span> <span class="n">secret</span> <span class="n">hostaddr</span><span class="o">=</span><span class="mf">127.0</span><span class="o">.</span><span class="mf">0.1</span> <span class="n">dbname</span><span class="o">=</span><span class="n">postgres</span> <span class="n">user</span><span class="o">=</span><span class="n">postgres</span>
</pre></div>
</div>
<p>You can then start the service at the command line using <em>net start pgAgent</em>, or from the <em>Services</em> control panel applet. Any logging output or errors will be reported in the Application event log. The DEBUG mode may be used to run pgAgent from a command prompt. When run this way, log messages will output to the command window.</p>
</div>
</div>
...
<li class="toctree-l2 current"><a class="current reference internal" href="#">Installing pgAgent</a><ul>
<li class="toctree-l3"><a class="reference internal" href="#database-setup">Database setup</a></li>
<li class="toctree-l3"><a class="reference internal" href="#daemon-installation-on-unix">Daemon installation on Unix</a></li>
<li class="toctree-l3"><a class="reference internal" href="#service-installation-on-windows">Service installation on Windows</a></li>
</ul>
</li>
